.PHONY: all build test clean run debug release format help client client-debug clean-client run-client benchmark benchmark-ci benchmark-clean
...
help:
	...
	@echo "  make benchmark-ci    - Run reduced TPC-C crash-recovery consistency test"
...
benchmark-ci:
	@$(MAKE) benchmark \
		TPCC_DB=tpcc_ci_db \
		TPCC_RESULT=benchmark/tpcc/ci_result.json \
		TPCC_WAREHOUSES=1 \
		TPCC_WORKERS=2 \
		TPCC_WARMUP=2 \
		TPCC_MEASURE=8 \
		TPCC_ROUNDS=1 \
		TPCC_PROGRESS_INTERVAL=2 \
		TPCC_RESTART_TIMEOUT=30
