...
public:
    ...
    void create_file(const std::string& filename, int record_size) {
        ...
        // We have: sizeof(RmPageHdr) + n * TUPLE_META_SIZE + (n+7)/8 + n * record_size <= PAGE_SIZE
        // => sizeof(RmPageHdr) + (n+7)/8 + n * (record_size + TUPLE_META_SIZE) <= PAGE_SIZE
        int effective_record_size = record_size + TUPLE_META_SIZE;
        file_hdr.num_records_per_page =
            (BITMAP_WIDTH * (PAGE_SIZE - 1 - (int)sizeof(RmFileHdr)) + 1) / (1 + effective_record_size * BITMAP_WIDTH);
