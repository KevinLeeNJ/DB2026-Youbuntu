...
class RoundResult:
    ...
    def total_committed_new_order(self) -> int:
        return sum(self.counts[phase]["new_order"]["commit"] for phase in ("warmup", "measure", "drain"))
    ...
    def abort_rate(self) -> float:
        ...
        for txn_counts in self.counts.values():
            for outcomes in txn_counts.values():
                ...
                aborted += sum(count for name, count in outcomes.items() if name != "commit")
        total = committed + aborted
...
def median_tpmc(rounds: list[RoundResult]) -> float:
    return 0.0 if not rounds else float(median(round_result.tpmc() for round_result in rounds))
...
def format_progress_line(
    ...
    abort_rate = 0.0 if counts["total"] == 0 else counts["aborts"] / counts["total"] * 100.0
    tpmc = 0.0
